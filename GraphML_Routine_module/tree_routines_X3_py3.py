...
# Updated to work with networkX 2.X -- many features of 1.X are broken by the upgrade. At the moment nx.edge has been replaced by nx.adj -- which should work with both 1.X and 2.X.  nx.nodes still works for now, but will apparently be broken in the future in networkX 3.X. 
...
def getBranchKeyInfo(nxTreeSurveyorGraph, TreeSurveyorBranchID):
    ...
    if TreeSurveyorBranchID == 'Root':
        ...
        print(nxTreeSurveyorGraph.nodes['Root'])
        ...
        tipsegment = name[:-2]
        print(nxTreeSurveyorGraph.nodes[tipsegment])
        print('Tip boundary location:', nxTreeSurveyorGraph.nodes[name])
        print(nxTreeSurveyorGraph.adj[tipsegment][name])
    elif len(TreeSurveyorBranchID) == 1:
        print(nxTreeSurveyorGraph.nodes[TreeSurveyorBranchID])
...
def getAllCoordinates(nxTreeSurveyorGraph):
    ...
    '''
    coordinateArrayList = []
    for branchID in nxTreeSurveyorGraph.nodes:
        ...
        # supply a networkX graph (from networkX library) from TreeSurveyor and a Branch ID as STRING, will return the coordinates of the END node of the branch
                xCoord = nxTreeSurveyorGraph.nodes[branchID]['Position X']
                yCoord = nxTreeSurveyorGraph.nodes[branchID]['Position Y']
                zCoord = nxTreeSurveyorGraph.nodes[branchID]['Position Z']
...
def plotKidneyHull(nxTreeSurveyorGraph):
    verts = getAllCoordinates(nxTreeSurveyorGraph)  # Assuming this function gets all the vertices
    ...
    for s in faces:
        ...
        tri = a3.art3d.Poly3DCollection([sq])
        tri.set_color(colors.rgb2hex(np.random.rand(3)))  # Random color for each face
        tri.set_edgecolor('k')
        tri.set_alpha(0.3)  # Set transparency for the faces
        ax.add_collection3d(tri)
        
        # Draw edges to connect vertices
        ax.plot([verts[s[0], 0], verts[s[1], 0]], [verts[s[0], 1], verts[s[1], 1]], [verts[s[0], 2], verts[s[1], 2]], color='k')
        ax.plot([verts[s[1], 0], verts[s[2], 0]], [verts[s[1], 1], verts[s[2], 1]], [verts[s[1], 2], verts[s[2], 2]], color='k')
        ax.plot([verts[s[2], 0], verts[s[0], 0]], [verts[s[2], 1], verts[s[0], 1]], [verts[s[2], 2], verts[s[0], 2]], color='k')
...
def getLocalBifurcationAngle(nxTreeSurveyorGraph, TreeSurveyorBranchID):
    # supply a networkX graph (from networkX library) from TreeSurveyor and a Branch ID, will return the branch diameter
    try: 
        branchLocalBifurcationAngle = nxTreeSurveyorGraph.nodes[TreeSurveyorBranchID]['Local Bifurcation Angle (degrees)']
    except:
        return
    # Return a FLOAT
...
def getGlobalBifurcationAngle(nxTreeSurveyorGraph, TreeSurveyorBranchID):
    # supply a networkX graph (from networkX library) from TreeSurveyor and a Branch ID, will return the branch diameter
    try:
        branchGlobalBifurcationAngle = nxTreeSurveyorGraph.nodes[TreeSurveyorBranchID]['Global Bifurcation Angle (degrees)']
    except:
        return
    # Return a FLOAT
...
def getLocalDihedralAngle(nxTreeSurveyorGraph, TreeSurveyorBranchID):
    # supply a networkX graph (from networkX library) from TreeSurveyor and a Branch ID, will return the branch diameter
    try:
        branchLocalDihedralAngle = nxTreeSurveyorGraph.nodes[TreeSurveyorBranchID]['Local Dihedral (p-)Angle (degrees)']
    except:
        return
    # Return a FLOAT
...
def getGlobalDihedralAngle(nxTreeSurveyorGraph, TreeSurveyorBranchID):
    # supply a networkX graph (from networkX library) from TreeSurveyor and a Branch ID, will return the branch diameter
    try:
        branchGlobalDihedralAngle = nxTreeSurveyorGraph.nodes[TreeSurveyorBranchID]['Global Dihedral (p-)Angle (degrees)']
    except:
        return
    # Return a FLOAT
...
def getBaseCoordinate(nxTreeSurveyorGraph, TreeSurveyorBranchID):
    ...
    if "Extension Of " in TreeSurveyorBranchID:
        ...
        parentSegment = TreeSurveyorBranchID[:-2]
    xCoord = nxTreeSurveyorGraph.nodes[parentSegment]['Position X']
    yCoord = nxTreeSurveyorGraph.nodes[parentSegment]['Position Y']
    zCoord = nxTreeSurveyorGraph.nodes[parentSegment]['Position Z']
...
def getEndCoordinate(nxTreeSurveyorGraph, TreeSurveyorBranchID):
    # supply a networkX graph (from networkX library) from TreeSurveyor and a Branch ID as STRING, will return the coordinates of the END node of the branch
    xCoord = nxTreeSurveyorGraph.nodes[TreeSurveyorBranchID]['Position X']
    yCoord = nxTreeSurveyorGraph.nodes[TreeSurveyorBranchID]['Position Y']
    zCoord = nxTreeSurveyorGraph.nodes[TreeSurveyorBranchID]['Position Z']
...
def getBranchSplineControlPoints(nxTreeSurveyorGraph, TreeSurveyorBranchID,cropped=False):
    ...
    if cropped==True:
        ...
        combolist= list(chain.from_iterable(zip(splineControlPointsX, splineControlPointsY,splineControlPointsZ)))
        i=0
        splineControlPoints=[]
...
def rgbArray_2_hsvArray(rgb_array):
    ...
    for x in list(range(dimensions[0])):
        for y in list(range(dimensions[1])):
            for z in list(range(dimensions[2])):
...
def removeYellowVoxelTree(rgb3DYellowSkeletonArray):
    ...
    for x in list(range(Xdim)):
        for y in list(range(Ydim)):
            for z in list(range(Zdim)):
...
def loadGraphML(GraphML_file):
    """
    Attempt to load a Tree Surveyor GraphML file. If the file contains non-numerical values like 
    "no bifurcation" or "#IND", which can cause errors, this function removes those lines and 
    attempts to load the cleaned file.
    
    Args:
    - GraphML_file (str): The path to the GraphML file to be loaded.
    
    Returns:
    - nxGraphFile: The loaded NetworkX graph object, or None if loading fails.
    """
    try:
        # Attempt to load the GraphML file as-is
        nxGraphFile = nx.read_graphml(GraphML_file, str)
        print("Loaded GraphML file successfully on the first attempt.")
    except (KeyError, IOError, ValueError) as e:
        # Handle specific exceptions related to file reading and parsing
        print(f"Error loading GraphML file: {e}. Attempting to clean the file and reload.")
        output_graph_buffer = io.StringIO()

        # Open the original GraphML file and clean it in-memory
        try:
            with open(GraphML_file, 'r') as inputGraphML:
                for line in inputGraphML:
                    # Only write lines that do not contain problematic strings
                    if 'no bifurcation' not in line and '#IND' not in line:
                        output_graph_buffer.write(line)
            
            # Rewind the buffer and attempt to load the cleaned GraphML data
            output_graph_buffer.seek(0)
            nxGraphFile = nx.read_graphml(output_graph_buffer, str)
            print("Cleaned and loaded GraphML file successfully.")
        except (IOError, ValueError, nx.NetworkXError) as inner_e:
            print(f"Failed to load cleaned GraphML file: {inner_e}")
            nxGraphFile = None
        finally:
            output_graph_buffer.close()

    # Check if the graph was loaded successfully
    if nxGraphFile is not None:
        print("GraphML file loaded successfully.")
        # You can add additional checks or return information here
    else:
        print("Failed to load the GraphML file.")
...
def addGenerationToGraphML(nxTreeSurveyorGraph):
    for i in nxTreeSurveyorGraph.nodes:
